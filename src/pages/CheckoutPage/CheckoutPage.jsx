...
export default function CheckoutPage() {
            ...
            <main role="main" id="main-content">
                <PageSection background="white" spacing="default">
                    <div className="checkout-empty">
                        ...
                        <Link to="/butik" className="btn-primary">Till butiken</Link>
                    </div>
                </PageSection>
            </main>
        );
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            setError(`Fel: ${err.message}. Kontrollera att backend körs på http://localhost:4242`);
            setLoading(false);
        }
    };

    const subtotal = getTotal();
    const shipping = 0; // Gratis frakt för nu
                                ...
                                {cart.map(item => (
                                    <div key={item.id} className="order-item">
                                        ...
                                        <div className="order-item-details">
                                            <h3>{item.name}</h3>
                                            <p className="order-item-quantity">Antal: {item.quantity}</p>
                                        </div>
